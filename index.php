<!doctype html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="">
    <meta name="author" content="">
    <title>Кинотеатр Иннополис</title>
    <link rel="icon" type="image/png" href="images/favicon.png" />
    <link rel="stylesheet" href="css/main.css?4" />
    <!-- <link rel="stylesheet" href="css/media.css" /> -->
    <link rel="stylesheet" href="css/owl.theme.default.min.css">
    <link rel="stylesheet" href="css/owl.carousel.min.css">
</head>

<body>
    <div class="block1">
        <div class="content">
            <div class="first-screen">
                <div class="logo-nav">
                    <div class="logo">
                        <a class="logo__img" href="https://university.innopolis.ru/" target="_blank" title="Университет Иннополиса">
                            <img alt="Университет Иннополиса" src="images/logo.png">
                        </a>
                    </div>
                    <div class="menu-wrap">
                        <div class="menu-burger">
                            <span></span>
                        </div>
                        <nav class="navigation">
                            <ul>
                                <li class="navigation__li">
                                    <a class="link-menu" href="#booking"> Заказать билет</a>
                                </li>
                                <li class="navigation__li">
                                    <a class="link-menu" href="#advantages_question">Преимущества </a>
                                </li>
                                <li class="navigation__li">
                                    <a class="link-menu" href="#films">Фильмы</a>
                                </li>
                                <li class="navigation__li">
                                    <a class="link-menu" href="#contacts">Контакты</a>
                                </li>
                            </ul>
                        </nav>
                    </div>
                    <div class="city">Ваш город:
                        <a href="/" id="city_name">-Не определен-</a>
                    </div>
                </div>
                <div id="modal_city">
                    <div class="modal" style="widows: 300px;">
                        <div class="conteiner3">
                            <button id="closeModalCity">X</button>
                            <div class="modal_city_your">Ваш город</div>
                            <input name="city_choose" type="text" id="">
                            <div id="result"></div>
                        </div>
                    </div>
                </div>
                <div class="descriptors">
                    <h1 class="descriptors__main">Закажите билет в кино</h1>
                    <div class="descriptors__second"> Всем покупателям подарок!</div>
                </div>
            </div>
        </div>
    </div>
    <div class="block2">
        <div class="content">
            <div class="advantages">
                <div class="welcome">
                    <h2>Добро пожаловать!</h2>
                    <p>Cumque dolorem eum harum laudantium libero molestias saepe soluta? Facere minima optio
                        perferendis
                        quibusdam quis reprehenderit, saepe! Debitis enim optio saepe voluptatibus.</p>
                    <p>Cumque dolorem eum harum laudantium libero molestias saepe soluta? Facere minima optio
                        perferendis
                        quibusdam quis reprehenderit, saepe! Debitis enim optio saepe voluptatibus.</p>
                </div>
                <h2 id="advantages_question">Почему мы?</h2>
                <div class="benefit-flex">
                    <div class="benefit">
                        <div class="benefit__screen">
                            <a class="benefit__screen1" href="#">
                                <img src="images/b2_icon_1.jpg" title="Большие экраны" alt="Большие экраны">
                            </a>
                            <h3 class="big1">Большие экраны</h3>
                            <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. At autem
                                cum dolores dolorum eius
                                error
                                expedita illum itaque molestiae mollitia nemo, nostrum officia quas ratione tempora
                                temporibus
                                veritatis
                                vero voluptates.</p>
                        </div>
                    </div>
                    <div class="benefit">
                        <div class="benefit__screen">
                            <a class="benefit__screen1" href="#">
                                <img class="benefit__screen2" src="images/b2_icon_2.jpg" title="Оъемный звук" alt="Объемный звук">
                            </a>
                            <h3 class="big2">Объемный звук</h3>
                            <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. At autem cum dolores dolorum
                                eius
                                error
                                expedita illum itaque molestiae mollitia nemo, nostrum officia quas ratione tempora
                                temporibus
                                veritatis
                                vero voluptates.</p>
                        </div>
                    </div>
                    <div class="benefit b3">
                        <div class="benefit__screen">
                            <a class="benefit__screen1" href="#">
                                <img class="benefit__screen3" src="images/b2_icon_3.jpg" title="Удобные кресла" alt="Удобные кресла">
                            </a>
                            <h3 class="big3">Удобные кресла</h3>
                            <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. At autem cum dolores dolorum
                                eius
                                error
                                expedita
                                illum itaque molestiae mollitia nemo, nostrum officia quas ratione tempora temporibus
                                veritatis
                                vero
                                voluptates.</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <br>
    <div id="booking" class="block3">
        <div class="content">
            <div class="movie">
                <h2>Выберите фильм</h2>
                <div class="table-wrap">
                    <table id="filmsHire" class="movie-table">
                        <thead>
                            <tr class="movie-header">
                                <th class="movie-header__h1"><a href="#">Начало сеанса</a></th>
                                <th class="movie-header__h2"><a href="#">Название фильма</a></th>
                                <th class="movie-header__h3"><a href="#">Жанр</a></th>
                            </tr>
                        </thead>
                        <tbody>
                        </tbody>
                        <tfoot>
                        </tfoot>
                    </table>
                </div>
                <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. At autem cum dolores dolorum eius error
                    expedita
                    illum itaque molestiae mollitia nemo, nostrum officia quas ratione tempora temporibus veritatis vero
                    voluptates.</p>
            </div>
        </div>
    </div>
    <div class="block4">
        <div class="content">
            <div class="gift">
                <img src="images/b4_img.png" title="Подарок" alt="Подарок">
                <h2 class="gift__promise">Подарок каждому!</h2>
                <div class="gift__conditions">
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ab aperiam aspernatur, cum, cupiditate
                        expedita libero mollitia nesciunt nihil nobis obcaecati quasi ratione reprehenderit sequi sint,
                        tenetur
                        unde velit veniam! Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ab aperiam
                        aspernatur, cum,
                        cupiditate excepturi expedita libero mollitia nesciunt nihil nobis obcaecati quasi ratione
                        reprehenderit
                        sequi sint, tenetur unde velit veniam!</p>
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ab aperiam aspernatur, cum, cupiditate
                        excepturi
                        expedita libero mollitia nesciunt nihil nobis obcaecati quasi ratione reprehenderit sequi sint,
                        tenetur unde
                        velit veniam!</p>
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ab aperiam aspernatur, cum, cupiditate
                        excepturi
                        expedita libero mollitia nesciunt nihil nobis obcaecati quasi ratione reprehenderit sequi sint,
                        tenetur unde
                        velit veniam!</p>
                </div>
                <button id="getPresentModal" class="botton">Получить подарок</button>
            </div>
        </div>
    </div>
    <div class="block5" id="films">
        <div id="films" class="block-films">
            <div class="interactive">
                <h2 class="interactive-films">Фильмы</h2>
                <p class="interactive-descriptions">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ab
                    adipisci alias, animi, commodi eius
                    ipsum, laudantium neque nihil omnis perferendis sed sequi tempore! Ad ea esse ex inventore
                    repudiandae,
                    suscipit!</p>
                <div id="sliderSection" class="interactive-flex owl-carousel">
                </div>
            </div>
        </div>
    </div>
    <div class="block6">
        <div id="contacts" class="content">
            <img src="https://via.placeholder.com/140*140.png">
        </div>
    </div>
    <div class="block7">
        <footer>
        </footer>
        <div id="modalPresent">
            <form class="ajax-form" id="form_with_file">
                <div id="order" class="modal" style="widows: 300px;">
                    <button id="closeModalPresent">X</button>
                    <div class="conteiner2">
                        <div>
                            <input id="name_guest" type="text" name="name" placeholder="Имя*" value="<?php echo isset($_COOKIE['name']) ? $_COOKIE['name'] : '' ?>" aria-required="true" aria-label="Введите имя">
                        </div>
                        <p class="input-error input-error_name"></p>
                        <?php if (isset($_GET['error_name']) && $_GET['error_name']) { ?>
                            <p class="error-input"><?php echo ('Поле обязательно к заполнению') ?></p>
                        <?php } ?>
                        <br>
                        <div>
                            <input id="phone_guest" type="tel" id="phone" name="phone" placeholder="+79999999999" value="<?php echo isset($_COOKIE['phone']) ? $_COOKIE['phone'] : '' ?>" aria-required="true" aria-label="Введите телефон">
                        </div>
                        <br>
                        <div>
                            <input type="email" placeholder="email" name="email" aria-label="Введите email" value="<?php echo isset($_COOKIE['email']) ? $_COOKIE['email'] : '' ?>">
                        </div>
                        <?php if (isset($_GET['error_email']) && $_GET['error_email']) { ?>
                            <p class="error-input"><?php echo ('Допускается указывать почту в доменах *.com и *.org') ?></p>
                        <?php } ?>
                        <br>
                        <div>
                            <select id="select_guest" aria-label="Выберете одну из десяти выпадающих позиций" name="present" aria-required="true">
                                <option value="">Выберите подарок</option>
                                <option value="Билет в подарок">Билет в подарок</option>
                                <option value="Попкорн">Попкорн</option>
                                <option value="Газировка">Газировка</option>
                            </select>
                            <br>
                            <br>
                            <div aria-label="Выберете одну из двух позиций" name="client_choice2">
                                <input id="one" type="radio" name="popcorn">
                                <label for="one">Люблю попкорн</label>
                                <br>
                                <input id="two" type="radio" name="popcorn">
                                <label for="two">Не люблю попкорн</label>
                            </div>
                            <br>
                            <div aria-label="Поставьте галочку если вы согласны с обработкой персональных данных" aria-required="true">
                                <input type="hidden" name="agree" value="">
                                <input id="agree" type="checkbox" name="agree" class="checkbox-required">
                                <label for="agree"> Я согласен на обработку персональных данных</label>
                            </div>
                            <br>
                            <div>
                                <input type="hidden" name="MAX_FILE_SIZE" value="300000">
                                <input id="file" class="file1" type="file" accept="image/jpeg, image/png, image/gif" enctype="multipart/form-data" aria-label="Загрузите файл" name="client_file">
                                <label id="file-label" for="file">Выберите файл</label>
                            </div>
                            <?php if (isset($_GET['error_form']) && $_GET['error_form']) { ?>
                                <div class="error-input">
                                    <img src="uploads/<?php $name ?>">
                                </div>
                            <?php } ?>
                        </div>
                        <br>
            </form>
            <div>
                <button id="sendPresentInfo" type="submit">Заказать</button>
                <button id="nextTry" type="submit">Отправить повторно</button>
            </div>
        </div>
    </div>
    </div>
    </div>
    <div id="orderForm">
        <div class="modal">
            <button id="closeOrderForm">X</button>
            <div class="conteiner2">
                <div>
                    <span>Название фильма:</span>
                    <span id="orderFilmName"></span>
                </div>
                <div>
                    <span>Начало сеанса:</span>
                    <span id="orderFilmStart"></span>
                </div>
                <div>
                    <span>Жанр:</span>
                    <span id="orderFilmGanar"></span>
                </div>
                <div>
                    <span>Стоимость билета:</span>
                    <span id="orderFilmPrice"></span>
                    <br>
                    <span>Количество билетов:</span>
                    <input type="number" min=1 id="orderFilmCountTicket">
                </div>
                <div>
                    <span>Общая стоимость билетов:</span>
                    <span id="orderFilmTotalPrice"></span>
                </div>
                <div>
                    <span>Номер телефона:</span>
                    <input type="tel" pattern="8-[0-9]{3}-[0-9]{7}" id="orderFilmPhone">
                </div>
                <div></div>
                <span>Имя:</span>
                <input type="text" id="orderFilmNameGuest">
            </div>
            <div>
                <button id="sendOrder" class="sendOrder" type="botton">Забронировать</button>
            </div>
            <div class="places"></div>
            <div class="placesNew"></div>
            <div>
                <span>Ваш выбор:</span>
                <span class="clientBrone"></span>
            </div>
        </div>
    </div>
    </div>
    </div>
    <footer>
        <div class="block9">
            <div class="content">
                <div class="contacts">
                    <div class="circle">
                        <a class="circle1" href="https://ru-ru.facebook.com/">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewbox="0 0 48 48">
                                <defs>
                                    <filter filterUnits="userSpaceOnUse" id="Filter_1" x="0px" y="0px">
                                        <feOffset in="SourceAlpha" dx="0" dy="2" />
                                        <feGaussianBlur result="blurOut" stdDeviation="1" />
                                        <feFlood flood-color="rgb(0, 0, 0)" result="floodOut" />
                                        <feComposite operator="atop" in="floodOut" in2="blurOut" />
                                        <feComponentTransfer>
                                            <feFuncA type="linear" slope="0.25" />
                                        </feComponentTransfer>
                                        <feMerge>
                                            <feMergeNode />
                                            <feMergeNode in="SourceGraphic" />
                                        </feMerge>
                                    </filter>
                                    <filter filterUnits="userSpaceOnUse" id="Filter_2" x="0px" y="0px">
                                        <feGaussianBlur result="blurOut" stdDeviation="1" />
                                        <feFlood flood-color="rgb(0, 0, 0)" result="floodOut" />
                                        <feComposite operator="atop" in="floodOut" in2="blurOut" />
                                        <feComponentTransfer>
                                            <feFuncA type="linear" slope="0.25" />
                                        </feComponentTransfer>
                                        <feMerge>
                                            <feMergeNode />
                                            <feMergeNode in="SourceGraphic" />
                                        </feMerge>
                                    </filter>
                                </defs>
                                <g filter="url(#Filter_1)">
                                    <path fill-rule="evenodd" fill="rgb(255, 255, 255)" d="M23.500,44.999 C11.073,44.999 1.000,34.926 1.000,22.500 C1.000,10.073 11.073,-0.000 23.500,-0.000 C35.926,-0.000 46.000,10.073 46.000,22.500 C46.000,34.926 35.926,44.999 23.500,44.999 ZM24.044,10.170 C18.962,10.170 20.120,16.199 19.951,18.397 C19.951,18.414 16.844,18.397 16.844,18.397 L16.844,22.488 L19.942,22.488 L19.942,36.884 L25.089,36.884 L25.078,22.488 L28.526,22.488 L29.202,18.375 L25.089,18.420 C25.089,15.500 24.863,14.312 26.570,14.312 C26.723,14.312 29.219,14.306 29.219,14.306 L29.224,10.170 L24.044,10.170 Z" />
                                </g>
                                <g filter="url(#Filter_2)">
                                    <path fill-rule="evenodd" fill="rgb(255, 255, 255)" d="M23.500,44.999 C11.073,44.999 1.000,34.926 1.000,22.500 C1.000,10.073 11.073,-0.000 23.500,-0.000 C35.926,-0.000 46.000,10.073 46.000,22.500 C46.000,34.926 35.926,44.999 23.500,44.999 ZM24.044,10.170 C18.962,10.170 20.120,16.199 19.951,18.397 C19.951,18.414 16.844,18.397 16.844,18.397 L16.844,22.488 L19.942,22.488 L19.942,36.884 L25.089,36.884 L25.078,22.488 L28.526,22.488 L29.202,18.375 L25.089,18.420 C25.089,15.500 24.863,14.312 26.570,14.312 C26.723,14.312 29.219,14.306 29.219,14.306 L29.224,10.170 L24.044,10.170 Z" />
                                </g>
                            </svg>
                        </a>
                        <a class="circle1" href="https://twitter.com/twitter">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewbox="0 0 48 48">
                                <defs>
                                    <filter filterUnits="userSpaceOnUse" id="Filter_1" x="0px" y="0px">
                                        <feOffset in="SourceAlpha" dx="0" dy="1" />
                                        <feGaussianBlur result="blurOut" stdDeviation="1" />
                                        <feFlood flood-color="rgb(0, 0, 0)" result="floodOut" />
                                        <feComposite operator="atop" in="floodOut" in2="blurOut" />
                                        <feComponentTransfer>
                                            <feFuncA type="linear" slope="0.25" />
                                        </feComponentTransfer>
                                        <feMerge>
                                            <feMergeNode />
                                            <feMergeNode in="SourceGraphic" />
                                        </feMerge>
                                    </filter>

                                </defs>
                                <g filter="url(#Filter_1)">
                                    <path fill-rule="evenodd" fill="rgb(255, 255, 255)" d="M24.475,45.975 C12.049,45.975 1.975,35.901 1.975,23.475 C1.975,11.048 12.049,0.975 24.475,0.975 C36.901,0.975 46.975,11.048 46.975,23.475 C46.975,35.901 36.901,45.975 24.475,45.975 ZM36.437,21.249 C34.699,15.199 30.523,15.199 30.523,15.199 C30.523,15.199 32.296,14.193 32.244,13.580 C31.699,13.188 30.605,13.972 30.513,13.972 C30.789,13.778 30.625,13.226 30.267,13.017 C29.739,13.069 29.279,14.040 29.279,14.040 C29.279,14.040 29.020,13.563 28.785,13.563 C25.990,14.330 24.021,20.781 24.030,20.789 C14.903,14.628 14.298,15.813 14.298,15.830 C13.582,17.432 16.360,19.460 16.360,19.460 L15.566,19.484 C15.566,19.484 14.399,19.445 14.654,20.279 C14.914,21.344 16.298,22.448 16.309,22.460 C16.991,22.783 17.843,22.476 17.843,22.476 C17.843,22.476 16.940,22.948 16.368,23.305 C15.426,23.914 16.103,24.454 16.205,24.556 C17.432,25.868 19.360,25.783 19.360,25.783 C19.360,25.783 18.464,26.857 18.457,27.265 C18.457,27.930 19.053,28.340 19.104,28.390 C19.637,28.755 20.427,28.800 20.434,28.800 C15.495,34.032 9.543,28.769 9.543,28.748 C10.208,32.583 15.849,36.282 25.768,34.850 C33.812,33.572 36.386,24.760 36.386,24.760 C36.386,24.760 39.709,24.947 40.084,22.971 C39.045,23.158 37.545,22.852 37.545,22.852 C37.545,22.852 40.101,21.744 40.221,20.414 C38.721,21.573 36.488,21.249 36.437,21.249 Z" />
                                </g>
                            </svg>
                        </a>
                        <a class="circle1" href="https://www.instagram.com/">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewbox="0 0 48 48">
                                <defs>
                                    <filter filterUnits="userSpaceOnUse" id="Filter_1" x="0px" y="0px">
                                        <feOffset in="SourceAlpha" dx="0" dy="1" />
                                        <feGaussianBlur result="blurOut" stdDeviation="1" />
                                        <feFlood flood-color="rgb(0, 0, 0)" result="floodOut" />
                                        <feComposite operator="atop" in="floodOut" in2="blurOut" />
                                        <feComponentTransfer>
                                            <feFuncA type="linear" slope="0.25" />
                                        </feComponentTransfer>
                                        <feMerge>
                                            <feMergeNode />
                                            <feMergeNode in="SourceGraphic" />
                                        </feMerge>
                                    </filter>

                                </defs>
                                <g filter="url(#Filter_1)">
                                    <path fill-rule="evenodd" fill="rgb(255, 255, 255)" d="M23.535,45.071 C11.109,45.071 1.035,34.997 1.035,22.570 C1.035,10.144 11.109,0.071 23.535,0.071 C35.962,0.071 46.035,10.144 46.035,22.570 C46.035,34.997 35.962,45.071 23.535,45.071 ZM13.873,11.721 C12.424,11.721 11.250,12.896 11.250,14.345 C11.250,15.794 12.424,16.969 13.873,16.969 C15.322,16.969 16.496,15.794 16.496,14.345 C16.496,12.896 15.322,11.721 13.873,11.721 ZM16.653,18.624 L11.562,18.624 L11.562,33.990 L16.653,33.990 L16.653,18.624 ZM29.734,18.249 C26.611,18.280 25.666,20.076 25.182,20.810 L25.182,18.530 L20.026,18.530 L20.026,33.927 L25.179,33.927 L25.179,25.745 C25.179,25.745 24.810,22.590 27.958,22.590 C30.292,22.590 30.332,25.109 30.308,25.452 C30.306,25.291 30.304,25.270 30.301,25.464 C30.301,25.508 30.305,25.498 30.308,25.452 C30.319,26.501 30.301,33.958 30.301,33.958 L35.360,33.974 C35.360,33.974 35.423,25.948 35.423,25.916 C35.423,24.464 35.449,18.249 29.734,18.249 Z" />
                                </g>
                            </svg>
                        </a>
                        <a class="circle1" href="https://www.google.ru/">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewbox="0 0 48 48">
                                <defs>
                                    <filter filterUnits="userSpaceOnUse" id="Filter_1" x="0px" y="0px">
                                        <feOffset in="SourceAlpha" dx="0" dy="1" />
                                        <feGaussianBlur result="blurOut" stdDeviation="1" />
                                        <feFlood flood-color="rgb(0, 0, 0)" result="floodOut" />
                                        <feComposite operator="atop" in="floodOut" in2="blurOut" />
                                        <feComponentTransfer>
                                            <feFuncA type="linear" slope="0.25" />
                                        </feComponentTransfer>
                                        <feMerge>
                                            <feMergeNode />
                                            <feMergeNode in="SourceGraphic" />
                                        </feMerge>
                                    </filter>

                                </defs>
                                <g filter="url(#Filter_1)">
                                    <path fill-rule="evenodd" fill="rgb(255, 255, 255)" d="M24.475,45.950 C12.049,45.950 1.975,35.876 1.975,23.449 C1.975,11.023 12.049,0.949 24.475,0.949 C36.901,0.949 46.975,11.023 46.975,23.449 C46.975,35.876 36.901,45.950 24.475,45.950 ZM20.724,10.471 C17.281,10.471 15.706,11.388 14.660,12.282 C13.748,13.194 12.533,14.555 12.567,16.800 C12.567,18.741 14.001,22.105 18.103,22.121 C18.373,22.105 18.536,22.116 19.177,22.065 C18.958,22.571 18.862,22.887 18.845,23.359 C18.845,24.220 19.155,24.821 19.847,25.632 C17.872,25.834 15.343,25.941 13.433,27.207 C10.339,29.097 10.665,31.364 10.665,31.330 C10.902,36.309 18.108,36.208 18.086,36.174 C23.869,36.118 26.789,32.936 26.789,29.750 C26.789,28.035 26.104,26.452 24.160,25.048 C23.307,24.271 22.059,23.816 22.064,22.685 C22.064,20.626 25.855,20.183 25.855,16.198 C25.722,13.158 23.672,11.848 23.410,11.679 C23.465,11.679 25.511,11.680 25.511,11.680 L27.683,10.471 C27.683,10.471 20.758,10.471 20.724,10.471 ZM37.758,13.478 L34.586,13.478 L34.586,10.305 L32.470,10.305 L32.470,13.478 L29.297,13.478 L29.297,15.593 L32.470,15.593 L32.470,18.766 L34.586,18.766 L34.586,15.593 L37.758,15.593 L37.758,13.478 ZM19.438,21.208 C16.552,21.049 15.606,16.770 15.606,16.770 C15.606,16.770 14.484,11.716 18.530,11.671 C21.689,11.330 22.771,16.188 22.771,16.188 C22.771,16.188 23.916,21.208 19.438,21.208 ZM24.707,31.122 C24.623,31.257 25.095,34.559 19.694,34.762 C13.889,34.661 13.765,30.982 13.765,30.644 C13.741,26.808 18.910,26.618 20.240,26.576 C21.388,26.723 24.667,28.627 24.707,31.122 Z" />
                                </g>
                            </svg>
                        </a>
                        <a class="circle1" href="https://tlgrm.ru/">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewbox="0 0 48 48">
                                <defs>
                                    <filter filterUnits="userSpaceOnUse" id="Filter_1" x="0px" y="0px">
                                        <feOffset in="SourceAlpha" dx="0" dy="1" />
                                        <feGaussianBlur result="blurOut" stdDeviation="1" />
                                        <feFlood flood-color="rgb(0, 0, 0)" result="floodOut" />
                                        <feComposite operator="atop" in="floodOut" in2="blurOut" />
                                        <feComponentTransfer>
                                            <feFuncA type="linear" slope="0.25" />
                                        </feComponentTransfer>
                                        <feMerge>
                                            <feMergeNode />
                                            <feMergeNode in="SourceGraphic" />
                                        </feMerge>
                                    </filter>

                                </defs>
                                <g filter="url(#Filter_1)">
                                    <path fill-rule="evenodd" fill="rgb(255, 255, 255)" d="M23.499,44.999 C11.073,44.999 0.999,34.926 0.999,22.499 C0.999,10.073 11.073,-0.001 23.499,-0.001 C35.925,-0.001 45.999,10.073 45.999,22.499 C45.999,34.926 35.925,44.999 23.499,44.999 ZM30.994,30.137 C27.117,31.923 24.988,31.312 24.988,28.260 C24.988,26.670 24.992,21.993 24.992,21.993 L30.000,21.993 L30.000,17.981 L24.987,17.984 L24.987,11.959 C24.987,11.959 22.697,11.958 21.588,11.958 C21.402,14.735 20.468,16.307 17.014,18.005 C17.014,19.134 17.011,21.970 17.011,21.970 L20.000,21.970 C20.000,21.970 20.000,29.654 20.000,30.989 C20.000,33.629 22.408,34.654 24.119,34.904 C26.237,35.214 28.874,34.829 30.994,34.132 C30.994,32.886 30.994,31.757 30.994,30.137 Z" />
                                </g>
                            </svg>
                    </div>
                    </a>
                    <div class="goodbye">
                        <p>
                            Hope we see you at the event!
                            <br>
                            All Rights Reserved. Envato Bangladesh 2015

                        </p>

                    </div>
                </div>
            </div>
        </div>
    </footer>
    <script type="module" src="js/main.js?3"></script>
</body>

</html>